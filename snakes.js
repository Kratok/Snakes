...
function rgb(r, g, b) {
	return gfx.fillStyle = gfx.strokeStyle = "rgb(" + Math.floor(255 * r) + "," + Math.floor(255 * g) + "," + Math.floor(255 * b) + ")";
};
function rgba(r, g, b, a) {
	return gfx.fillStyle = gfx.strokeStyle = "rgba(" + Math.floor(255 * r) + "," + Math.floor(255 * g) + "," + Math.floor(255 * b) + "," + a + ")";
};
...
function bounce(lineRot, rot) {
	return (-2*(rot - lineRot) + rot);
};
console.log(bounce);
console.log(bounce(0,0));
...
function render() {
	...
	for (var k = 0; k < snakeList.length; k++) {
		...
		var dumbcheck = false;
		var newcheck = false;
		if (true && snake.ctlFor === 1) {
			var tail = snake.list[snake.list.length - 1];
			var nextPos = snake.list[snake.list.length - 2];
			...
			if (tail.x == snake.bound.x1 &&
				(nextPos.x > snake.bound.x1)) {
				snake.bound.x1 = nextPos.x;
				checkX1 = true;
			} else if (tail.x == snake.bound.x2 &&
				(nextPos.x < snake.bound.x2)) {
				snake.bound.x2 = nextPos.x;
				checkX2 = true;
			}
			if (tail.y == snake.bound.y1 &&
				(nextPos.y > snake.bound.y1)) {
				snake.bound.y1 = nextPos.y;
				checkY1 = true;
			} else if (tail.y == snake.bound.y2 &&
				(nextPos.y < snake.bound.y2)) {
				snake.bound.y2 = nextPos.y;
				checkY2 = true;
			}
			...
			for (var i = snake.list.length - 1; i > 0; i--) {
				...
				if (checkX1 && body.x < snake.bound.x1)
					snake.bound.x1 = body.x;
				else if (checkX2 && body.x > snake.bound.x2)
					snake.bound.x2 = body.x;
				if (checkY1 && body.y < snake.bound.y1)
					snake.bound.y1 = body.y;
				else if (checkY2 && body.y > snake.bound.y2)
					snake.bound.y2 = body.y;
			}
			...
			for (var j = 0; j < snakeList.length; j++) {
				if (j == k)
					continue;
				var snakeBox = snakeList[j].bound;
				if (true || ( nX > snakeBox.x1 &&
					nX < snakeBox.x2 &&
					nY > snakeBox.y1 &&
					nY < snakeBox.y2)){
					var altSnake = snakeList[j];
					//var col = altSnake.list.length-2;
					for(var col = altSnake.list.length-2; col > 0;
						col -=2){
						var snakeBit = altSnake.list[col];
						//rgb(0,1,0);
						//gfx.fillRect(snakeBit.x-10, snakeBit.y-10, 20,20);
						temp = col;
						if(snakeBit.x-10 < nX && snakeBit.x+10 > nX 
						&& snakeBit.y-10 < nY && snakeBit.y+10 > nY){
							newcheck = true;
							var adj = altSnake.list[col-1].x - altSnake.list[col+1].x;
							var opp = altSnake.list[col-1].y - altSnake.list[col+1].y;
							head.rot = bounce(Math.atan(opp/adj) , head.rot);
							recalc = 1;
						}
			...
			if (head.x < snake.bound.x1)
				snake.bound.x1 = head.x;
			else if (head.x > snake.bound.x2)
				snake.bound.x2 = head.x;

			if (head.y < snake.bound.y1)
				snake.bound.y1 = head.y;
			else if (head.y > snake.bound.y2)
				snake.bound.y2 = head.y;
		}
				

				//======RENDERING===============
				//rgb(0, 1, 0);
				//gfx.strokeRect(snake.bound.x1, snake.bound.y1,
				//	snake.bound.x2 - snake.bound.x1, snake.bound.y2 - snake.bound.y1);
				...
				for (var r = 0; r < reflect; r++) {
					...
					for (var i = snake.list.length - 1; i >= 0; i -= snakeDivision) {
						console.log(i);
						var body = snake.list[i];
						gfx.beginPath();
						//rgb(1 - i*.01, i*.01, 0);
						hue(k - i * .01);
						//if(newcheck) rgb(1,1,1);
